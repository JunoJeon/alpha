...
class App extends React.Component {
	...
	async fill() {
		...
		for(;;) {
			...
			let alpha = new Alpha();
			let v = this.state.surface[alpha.line-1][alpha.column-1];
			
			if(v.fg == 'black' && v.bg == 'black') {
				this.state.count++;
			}
	...
	render() {
		...
		return(
			<>
			<button disabled={this.state.disabled} onClick={event => this.btnCreate_click(event)}>Create</button>
			<hr/>
			<table border={1}>
				<thead>
					<tr>
				...
				<tbody>
					{
						this.state.surface.map((row, k) =>
							<tr key={k}>
								{
									row.map((v, k) =>
										<td style={{color:v.fg, background:v.bg}} key={k}>
											{v.ch}
										</td>
									)
								}			
